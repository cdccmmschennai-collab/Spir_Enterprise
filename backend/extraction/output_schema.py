...
OUTPUT_COLUMNS: list[dict] = [
    ...
    # ── Two computed columns — filled by post_processor_v2.py ─────────────────
    # POSITION NUMBER: 0010 for equipment header; 0010/0020/… for spares
    #                  cross-sheet continuation for same tag
    {"col": "POSITION NUMBER",
     "field": "position_number", "default": None, "width": 14},
 
    # OLD MATERIAL NUMBER/SPF NUMBER: exactly 18 chars
    #   = trimmed SPIR digits + "-" + sheet/line suffix
...
def row_from_dict(item: dict) -> list:
    ...
    Supports both new field names (tag, description) and old column names (TAG NO, etc.).
    """
    row = make_empty_row()
    for entry in OUTPUT_COLUMNS:
        col_idx = CI[entry["col"]]
        # Try new field name first, then old column name, then default
